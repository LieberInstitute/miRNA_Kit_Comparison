...
#Pheno<- read.table(here("Brain_samples_missingClontech_batch2/Pheno_brain_full_May22.txt"), header = T)
...
miR_counts<-read.table(here("Brain_samples_missingClontech_batch2/miR.Counts.csv"), header = TRUE, sep = ",")
...
load(here("miRNA_hsa_GC.rda"))#this does not appear to be in the same order as miR_1000_raw... think this is in part because they have different lengths
load(here("miRNA_hsa_lengths.rda"))
load(here("UnionInfo.rda"))
...
split_kit_thresh$Five_double<-split_kit$Five_double[thresh$Five_double,]
...
split_batch_thresh$Five_double1000_Batch1<-split_batch$Five_double1000_Batch1[thresh$Five_double1000_Batch1,]
split_batch_thresh$Five_double1000_Batch2<-split_batch$Five_double1000_Batch2[thresh$Five_double1000_Batch2,]
...
#Clontech not ready 
pval_single<- pval_single[-grep("Clontech", names(pval_single))]
...
raw_table
#percent_table_noBatcheffect<-(raw_table[which(rownames(raw_table) == "TRUE"),] / colSums(raw_table))*100 # fIve_double had now batch effects....
...
library(ggplot2)
library(reshape2)
Names_thresh<-intersect(intersect(intersect(intersect(rownames(split_kit_thresh[[2]]), rownames(split_kit_thresh[[3]])), rownames(split_kit_thresh[[4]])), rownames(split_kit_thresh[[5]])), rownames(split_kit_thresh[[6]]))
...
colnames(qvals) <-c("Illumina", "NEB", "NEXTflex", "Deduped", "Five_double")
...
exp_avgs <- exp_avgs[2:6] #### delete when clontech is added
...
get_cor_results(data1 = expression_df, data2 = qvals_raw, test_names = test_names$names.split_kit_thresh.[2:6])
...
test_names <- data.frame(names(split_kit_thresh))
get_cor_results(data1 = expression_df, data2 = ranks_number, test_names = test_names$names.split_kit_thresh.[2:6]) # go with below with clontech
