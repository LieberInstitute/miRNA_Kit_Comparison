...
varexp_PctExp<- varexp_PctExp[-grep("Residual",rownames(varexp_PctExp)),]

pheatmap(varexp_PctExp, main = "Percent of batch error variance explained", cluster_cols = FALSE, cluster_rows = FALSE)
...
ggplot(exp_error, aes(x = Expression, y = Error, col = Kit)) + geom_point()+ stat_smooth(method = "loess")+ facet_grid(Kit~.) + theme(legend.position = "none", axis.text.x = element_text(size =17, angle = 60, hjust = 1), axis.text.y = element_text(size =15, hjust = 1), axis.title=element_text(size = 20), plot.title = element_text(size = 30), strip.text.x = element_text(size = 20)) + labs( x= "Expression", y = " Error across Batch", title = " Relationship of Error and Expression") 
```
...
errorFirst<-data.frame(First_4_bases =pd$First_4_bases, Error)
aggdata <-aggregate(errorFirst[-1], by=list(errorFirst$First_4_bases), FUN=mean)
rownames(aggdata) <- aggdata$Group.1
aggdata<-aggdata[which(colnames(aggdata)!="Group.1")]

pheatmap(aggdata,cluster_cols = FALSE, main = "Batch error of sequences with different first base")
pheatmap(aggdata,cluster_cols = FALSE,cluster_rows = FALSE, main = "Batch error of sequences with different first base")
