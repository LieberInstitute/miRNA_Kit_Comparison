...
# isomiR_Counts <-read.table(here("isomirs.csv"), header = TRUE, sep = ",", row.names = NULL)
...
#save(annotation, file = here("IsomiR_data/annotation.rda"))
...
# 
# library(XML)
# report_url<-here("IsomiR_data/report.html")# from miRge
...
#save(iso_Raw, file = here("IsomiR_data/Iso_raw.rda"))
#save(total_miRNA, report, file = here("IsomiR_data/total_miRNA.rda"))
```
...
load(here("IsomiR_data/Iso_raw.rda"))
load(here("IsomiR_data/annotation.rda"))
...
ggplot(data = isomiR_DATA, aes(x = StartingAmt, y = total_isomiRs, group = StartingAmt)) + geom_jitter()+ geom_boxplot(aes(fill = Kit)) + facet_grid(.~Kit) + scale_x_continuous(breaks=unique(isomiR_DATA$StartingAmt)) + theme(legend.position = "none", axis.text.x = element_text(angle = 90, hjust = 1)) + labs(y = " Number of Uniquely Detected miRNA Sequences", title = "miRNA Detection across kits") +geom_smooth(method = "lm", se=TRUE, color="black", aes(group=1))
...
# split_isoData <- list()
# for(i in unique(isomiR_DATA$StartingAmt)) {
#   split_isoData[[i]] <- data.frame(isomiR_DATA[which(isomiR_DATA$StartingAmt == i),])
# }

# split_kit_isoData <- list()
# for(i in unique(isomiR_DATA$StartingAmt)) {
#   for(kit in unique(isomiR_DATA$Kit)){
#   split_kit_isoData[[i]][kit] <- data.frame(isomiR_DATA[which(isomiR_DATA$StartingAmt == i & isomiR_DATA$Kit == kit),])}
# }
...
1) number of identified isomirs
2) identity of identified isomirs
3) expression of identified isomirs within a kit across triplicates
