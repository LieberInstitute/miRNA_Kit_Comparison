...
Pairwise comparisons of isomiR detection using all starting amounts and threshold across all triplicates
```{r}
library(dplyr)
library(reshape2)
library(compute.es)
...
How consistent are the results using all starting amounts and create 1000ng venn diagram
```{r}
...
library(VennDiagram)
Isomirseqs <-list(Clontech = Clontech_1000_isomirs, Illumina = Illumina_1000_isomirs, NEB = NEB_1000_isomirs, NEXTFlex = NEXTflex_1000_isomirs, Deduped = Deduped_1000_isomirs, Fivepercent = Fivepercent_1000_isomirs)
...
#Venn Diagram of Isomirs detected across 1000ng samples
...
Stats and plots for 1000ng data 
...
across_1000$sd <-aggregate(raw_det_1000$value, by = list(raw_det_1000$Kit), FUN = sd)
across_1000$mean <- aggregate(raw_det_1000$value, by = list(raw_det_1000$Kit), FUN = mean)
across_1000$up <-across_1000$mean$x+across_1000$sd$x
across_1000$down <-across_1000$mean$x-across_1000$sd$x

pdf(file =here("Figures/Raw_plots/isomiR_1000ng_Det_error_bars.pdf"),width=4,height=6, onefile=FALSE)
ggplot(across_1000, aes(x =Kit, y=Detection, fill = Kit)) + geom_bar(stat = "identity", aes(y =Detection, fill = Kit), color = "black") + theme(legend.position = "none", axis.text.x = element_text(size =17, angle = 60, hjust = 1, color = "black"), axis.text.y = element_text(size =18, hjust = 1, color = "black"), axis.title.y=element_text(size = 20)) + labs(y = " Detected Unique isomiRNA Sequences") +coord_cartesian(ylim=c(200,900))+ geom_text(aes(label = (Detection), size = 6, hjust = 0.5, vjust = 1.5) ) + geom_jitter(data = raw_det_1000, aes(x = Kit, y =value),color = "black", width = 0.02) +geom_errorbar(aes(ymin =across_1000$up, ymax=across_1000$down, x = across_1000$Kit, y = across_1000$Detection),color = "black", position=position_dodge(width = 0.9))
dev.off()
