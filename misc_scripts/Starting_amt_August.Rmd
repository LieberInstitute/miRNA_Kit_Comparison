...
anova(lm(split_errorAll$Clontech$value~ split_errorAll$Clontech$Starting_Amount))
anova(lm(split_errorAll$Illumina$value~ split_errorAll$Illumina$Starting_Amount))
anova(lm(split_errorAll$NEB$value~ split_errorAll$NEB$Starting_Amount))
anova(lm(split_errorAll$NEXTflex$value~ split_errorAll$NEXTflex$Starting_Amount))
anova(lm(split_errorAll$Deduped$value~ split_errorAll$Deduped$Starting_Amount))
anova(lm(split_errorAll$Fivepercent$value~ split_errorAll$Fivepercent$Starting_Amount))
#also want to know when is there more error - at what starting amount for each kit does it appear to be better
# for(kit in errorAll$Method){
#  split_errorAll[[kit]]<- errorAll[which(errorAll$Method == kit),]
#  for(amount in errorAll)}
...
colnames(ttestStats_across)<-tested_kits
ttestStats_across #lower t indicates lower error across triplicates
#maybe look at how the mean of the triplicates for each amounts is correlated to the mean of the other amounts- but with the raw data - not the error?
