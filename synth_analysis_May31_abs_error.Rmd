...
# abs_errorLast<-cbind(seqs_last, error_synth[1:6])
# abs_errorFirst<-cbind(seqs_first, error_synth[1:6])
# get_perc<-function(x) {(x/mean(x))*100}
...
#general error (not expression) without direction of first base and last base

```{r}
abs_errorLast<-cbind(seqs_last, error_synth[1:6])
abs_errorFirst<-cbind(seqs_first, error_synth[1:6])
get_perc<-function(x) {(x/mean(x))*100}
abs_errorLast<-data.frame(abs_errorLast[1], lapply(abs_errorLast[2:7], get_perc))
abs_errorFirst<-data.frame(abs_errorFirst[1], lapply(abs_errorFirst[2:7], get_perc))
...
GC stats of relative expression without ebayes 
```{r}
GC<-errorGC$GC
...
for(i in GCdata$Var1) { 
  split_GC[[i]] <- data.frame(GCdata[which(GCdata$Var1==i),])
} 
 run_anova_length <-function(x){anova(lm(x[[3]]~x[[2]]))}
GC_f_stats<-lapply(split_GC,run_anova_length)

GC_f_stats<-data.frame(t(data.frame(lapply(GC_f_stats, get_fStats))))
GC_f_stats
```
...
Fold stats without ebayes
```{r}

rownames(errorFoldG) <-NULL

data_foldG <-as.matrix(t(errorFoldG[-1]))
colnames(data_foldG)<-FoldG$FoldG
FoldGdata<-melt(data_foldG)
 split_fold <- list() 

for(i in FoldGdata$Var1) { 
  split_fold[[i]] <- data.frame(FoldGdata[which(FoldGdata$Var1==i),])
} 
 run_anova_length <-function(x){anova(lm(x[[3]]~x[[2]]))}
foldg_f_stats<-lapply(split_fold,run_anova_length)

foldg_f_stats<-data.frame(t(data.frame(lapply(foldg_f_stats, get_fStats))))
foldg_f_stats
```
