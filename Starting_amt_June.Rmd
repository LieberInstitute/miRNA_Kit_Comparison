...
make_plot <- function(data) {
    data$Starting_Amount <- factor(data$Starting_Amount, levels =c("100", "250", "500","1000" ,"1500", "2000"), order = TRUE)
  plot1000<<-ggplot(data = data, aes(x = Starting_Amount, y = value)) +geom_jitter(color = cols[1],aes( alpha =.7)) + geom_boxplot( outlier.shape = NA)+ geom_smooth(method = "loess", se=TRUE, color="black", aes(group=1))+
  theme(axis.title.x = element_text(size =0), 
...
make_plot(data = split_errorAll$Clontech)
```
