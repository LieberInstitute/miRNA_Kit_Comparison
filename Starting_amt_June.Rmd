...
get_perc_diff <-function(data) {
  ...
  tested_kits<<-list()
  for(i in names(test_names)){
    percresults[[i]]<<-((colMeans(data)[ which(names(colMeans(data))==test_names[i][2,])]-colMeans(data)[ which(names(colMeans(data))==test_names[i][1,])])/ colMeans(data)[ which(names(colMeans(data))==test_names[i][1,])])*100
    tested_names1[[i]]<<-test_names[i][1,]
    tested_names2[[i]]<<-test_names[i][2,]
    tested_kits[[i]] <<-paste0(tested_names1[[i]], "&", tested_names2[[i]])
    }
