...
#pdf(file ="/Users/carriewright/Documents/miRNA seq Projects/miRNA_gel_vs_gel_free_2019_analysis/kit_comp_stackedbar_percentages.pdf",width=10,height=8)
...
pdf(file =here("Figures/Raw_plots/mapped_types_barplot_1000_zoom.pdf"),width=2,height=9)
ggplot(aggregate_1000, aes(x = Method, y = x, color = Method))  +geom_bar(stat  = "identity", position = position_dodge(width = 0.9), aes(y =x, fill = Method), color = "black") +geom_errorbar(aes(ymin =aggregate_1000$x-aggregate_1000$sd, ymax=aggregate_1000$x +aggregate_1000$sd),color = "black", position=position_dodge(width = 0.9))+ facet_grid(aggregate_1000$Read_type ~., scales = "free")+ geom_jitter(data = mp1000, aes(x = Method, y =value),color = "black", width = 0.02) + facet_grid(Read_type ~., scales = "free") +theme( axis.title.x=element_blank(), axis.text.x=element_text( size = 15, angle = 60, face = "bold", colour = "black", hjust = 1), axis.text.y=element_text(size = 15, colour = "black"), axis.title.y=element_text(size = 20), legend.position = "none", strip.text.x = element_text(size = 16)) +ylab("Percentage of reads")  +scale_fill_manual(values = cols)
dev.off() 
