...
get_perc_diff <-function(data) {
  ...
  for(i in names(test_names)){
    percresults[[i]]<<-((colMeans(data)[ which(names(colMeans(data))==test_names[i][1,])]-colMeans(data)[ which(names(colMeans(data))==test_names[i][2,])])/ colMeans(data)[ which(names(colMeans(data))==test_names[i][1,])])*100
...
expressions <-lapply(IsomiR_expression, `[`, 2)
Mean_express<-data.frame(lapply(expressions, colMeans))
...
get_test_results<- function(data,test_names) {
  ...
  for(i in names(test_names)){
    ...
    # colnames(Kit1)<-c("error")
    # colnames(Kit2)<-c("error")
    tresults[[i]]<<-t.test(x=Kit1[[1]], y=Kit2[[1]], paired = TRUE) ### may have messed things up adding paired = TRUE previously had more ))
    tested_kits <<-paste0(tested_names1, "&", tested_names2)
  }
}
library(compute.es)
