...
grep("N", Clontech_100$sequence)
Illumina_100<-Illumina_100[-grep("N", Illumina_100$sequence),]
NEB_100 <-NEB_100[-grep("N", NEB_100$sequence),]
grep("N", NEXTflex_100$sequence)
grep("N", Deduped_100$sequence)
grep("N", Fivepercent_100$sequence)
...
stats on each canonical synth miRNA - how many isomirs
```{r}
...
###NEED to fix below!!!
Illumina_100_none<-data.frame(Kit = rep("Illumina", length(pd$name[which(!pd$name %in% Illumina_100$miRNA)])), Counts = rep(0, length(pd$x[which(!pd$name %in% Illumina_100$miRNA)])), sequence = pd$x[which(!pd$name %in% Illumina_100$miRNA)], miRNA =pd$name[which(!pd$name %in% Illumina_100$miRNA)])
Illumina_100_full<-rbind(Illumina_100, Illumina_100_none)
Illumina_100_full<- Illumina_100_full[order(match(Illumina_100_full$miRNA, seqs$name)),]

NEB_100_none<-data.frame(Kit = rep("NEB", length(pd$x[which(!pd$name %in% NEB_100$miRNA)])), Counts = rep(0, length(pd$x[which(!pd$name %in% NEB_100$miRNA)])), sequence = pd$x[which(!pd$name %in% NEB_100$miRNA)], miRNA =pd$name[which(!pd$name %in% NEB_100$miRNA)])
NEB_100_full<-rbind(NEB_100, NEB_100_none)
NEB_100_full<- NEB_100_full[order(match(NEB_100_full$miRNA, seqs$name)),]

NEXTflex_100_none<-data.frame(Kit = rep("NEB", length(pd$x[which(!pd$name %in% NEXTflex_100$miRNA)])), Counts = rep(0, length(pd$x[which(!pd$name %in% NEXTflex_100$miRNA)])), sequence = pd$x[which(!pd$name %in% NEXTflex_100$miRNA)], miRNA =pd$name[which(!pd$name %in% NEXTflex_100$miRNA)])
NEXTflex_100_full<-rbind(NEXTflex_100, NEXTflex_100_none)
NEXTflex_100_full<- NEXTflex_100_full[order(match(NEXTflex_100_full$miRNA, seqs$name)),]

Deduped_100_none<-data.frame(Kit = rep("Deduped", length(pd$x[which(!pd$name %in% Deduped_100$miRNA)])), Counts = rep(0, length(pd$x[which(!pd$name %in% Deduped_100$miRNA)])), sequence = pd$x[which(!pd$name %in% Deduped_100$miRNA)], miRNA =pd$name[which(!pd$name %in% Deduped_100$miRNA)])
Deduped_100_full<-rbind(Deduped_100, Deduped_100_none)
Deduped_100_full<- Deduped_100_full[order(match(Deduped_100_full$miRNA, seqs$name)),]

Fivepercent_100_none<-data.frame(Kit = rep("Fivepercent", length(pd$x[which(!pd$name %in% Fivepercent_100$miRNA)])), Counts = rep(0, length(pd$x[which(!pd$name %in% Fivepercent_100$miRNA)])), sequence = pd$x[which(!pd$name %in% Fivepercent_100$miRNA)], miRNA =pd$name[which(!pd$name %in% Fivepercent_100$miRNA)])
Fivepercent_100_full<-rbind(Fivepercent_100, Fivepercent_100_none)
Fivepercent_100_full<- Fivepercent_100_full[order(match(Fivepercent_100_full$miRNA, seqs$name)),]
...
yGene <-as.matrix(Clontech_100_full$x)
