...
ggplot(detected, aes(x =det_Data.startingAmt, y=det_Data.not_consistent, fill = det_Data.kit)) + geom_boxplot()+ facet_grid(.~det_Data.kit) + theme(legend.position = "none") + labs( x= "Starting Amount", y = " Percentage of Inconsistently Detected miRNA Sequences", title = "Inconsistentcy of miRNA Detection across kits") +geom_smooth(method = "lm", se=TRUE, color="black", aes(group=1))
