...
load(here("Complete_data/IsomiR_data/norm_miR.rda"))
load(here("Complete_data/IsomiR_data/total_miRNA.rda"))
load(here("Complete_data/IsomiR_data/Pheno.rda"))
...
for(i in unique(Pheno$startingAmt)) {
  ...
  split_kit_isoData[[i]][[kit]] <- data.frame(norm_miR[which(Pheno$startingAmt == (i) & Pheno$Kit == as.character(kit))])}
}
...
test <-split_kit_isoData$`1000`$Clontech[thresh$`1000`$Clontech,]
...
#save(thresh_number, file = here("Complete_data/IsomiR_data/full_thresh_number.rda") )
```

plot of threshold across Triplicates
```{r}
# load(here("Complete_Data/IsomiR_data/full_thresh_number.rda"))
...
# 
# #need to repeat some 4 times... with second batch
...
# NEB<-data.frame(t=unlist(lapply(thresh_kits$NEB[1:6], to_repeat)))
...
# 
# pdf(file =here("Figures/Raw_plots/isomiR_inconsistency_2batches.pdf"),width=10,height=6, onefile=FALSE)
